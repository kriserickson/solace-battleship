...
All the code used in this Codelab can be found in [this GitHub repository](https://github.com/solacese/battleship).
...
![Battleship Image](assets/battleship-board.jpg "Battleship Image")
...
![Solace Cloud Landing Page](assets/landing-page-signup.png "Solace Cloud Landing Page")

Fill out all the details for your messaging service, and then click "Create" at the bottom of the page.

![Create Solace Cloud Messaging Service](assets/create-service.png "Solace Cloud Messaging Service")
...
![Connect Tab Preview](assets/service-summary-page.png "Connect Tab Preview")
...
![Solace Web Messaging Tab](assets/smf-web-messaging-connect-tab.png "Solace Web Messaging Tab")
...
Assuming you have [git](https://git-scm.com) installed, clone the [solace-battleship](https://github.com/solacese/battleship) repo by running :

```
git clone https://github.com/solacese/battleship
```
...
Sync the following branch [battleship-lesson-1-connect-and-publish](https://github.com/solacese/battleship/tree/battleship-lesson-1-connect-and-publish) using the following command:
...
![Code Structure](assets/lesson-01-code-structure.png)
...
![Connect to Solace](assets/lesson-01-connect-to-solace.png "Connect to Solace")
...
![Landing-Page](assets/lesson01-landing-page.png)
...
![Publish a Join Event](assets/lesson-01-publish-join-event.png "Publish a Join Event")
...
![Test-Subscribe](assets/lesson-01-test-subscribe.png)
...
![Subscribe Result](assets/lesson-01-test-subscribe-result.png)
...
To see the solutions to this excercise, sync the following branch [battleship-lesson-1-connect-and-publish-solution](https://github.com/solacese/battleship/tree/battleship-lesson-1-connect-and-publish-solution) using the following command:
...
Sync the following branch [battleship-lesson-2-subscribe-and-page-transitions](https://github.com/solacese/battleship/blob/battleship-lesson-2-subscribe-and-page-transitions/) using the command:
...
![Subscribe to Join and Publish](assets/lesson-02-subscribing-join-and-publishing-game-start.png)
...
![Subscribe to Game Start](assets/lesson-02-subscribe-to-game-start.png)
...
![Publish BOARD SET events](assets/lesson-02-publish-board-set.png)
...
![Subscribe to Board Set](assets/lesson-02-subscribe-to-board-set.png)
...
![Lesson-02-Summary](assets/lesson-02-summary.png)
...
Sync the following branch [battleship-lesson-3-match-and-game-over](https://github.com/solacese/battleship/blob/battleship-lesson-3-match-and-game-over/) using the command:
...
![Match Start Events](assets/lesson-03-subscribe-to-match-start.png "Match Start Event")
...
![Board-Set](assets/board-set.png "Board Set")
...
![Match screen](assets/match-screen.png "Match screen")
...
![Subscribe to Match-Request](assets/lesson-03-subscribe.png "Subscribe")
...
![Publish request](assets/lesson-03-publish.png "Publish")
...
![Match and Game Over](assets/lesson-03-match-and-game-over.png "Match and Game Over")
...
To see the completed code for this section, sync the following branch [battleship-lesson-3-match-and-game-over-solution](https://github.com/solacese/battleship/blob/battleship-lesson-3-match-and-game-over-solution/) using the command:
...
Sync the following branch [battleship-lesson-4-enabling-multiple-sessions](https://github.com/solacese/battleship/tree/battleship-lesson-4-enabling-multiple-sessions) using the following command:
...
![Multi-Session Landing Page](assets/lesson-04-landing-page-multi-session.png)

Here is an example of the same flow you built in the previous lesson with a generated session-id :

![Multi-Session](assets/lesson-04-multi-session.png "Single Session")
...
To see the completed code for this section, sync the following branch [battleship-lesson-4-enabling-multiple-sessions-solution](https://github.com/solacese/battleship/blob/battleship-lesson-4-enabling-multiple-sessions-solution) using the command:
...
Sync the following branch [battleship-lesson-5-building-a-dashboard](https://github.com/solacese/battleship/tree/battleship-lesson-5-building-a-dashboard) using the following command:
...
![Lesson-05-dashboard](assets/lesson-05-dashboard.png)
...
Sync the following branch [battleship-lesson-6-create-a-spring-cloud-stream-skeleton](https://github.com/solacese/battleship/tree/battleship-lesson-6-create-a-spring-cloud-stream-skeleton) using the following command:
...
![Get Started Java API](assets/lesson-06-java-api.png)

From there, retrieve the credentials from the `Connection Details`:

![Connection Detials](assets/lesson-06-java-api-credentials.png)
...
Sync the following branch [battleship-lesson-7-implement-a-join-request-handler-in-scs](https://github.com/solacese/battleship/tree/battleship-lesson-7-implement-a-join-request-handler-in-scs) using the following command:
...
- Create seperate Solace users and ACLs for the Controller, Player1 and Player2

### Github Branch

Sync the following branch [battleship-optional-1-creating-seperate-users](https://github.com/solacese/battleship/tree/battleship-optional-creating-seperate-users) using the following command:

`git checkout battleship-optional-1-creating-seperate-users`

Positive
: REMINDER: Make sure the "au run watch" script you setup in the previous section is still running. If not goto battleship_frontend/ and type the following command: `au run watch` .This will auto watch any changes to your files and automatically reload the page at http://localhost:12345 to pick up any changes

### Introducing Authentication and Authorization in Solace PubSub+

Solace has two layers of security in Solace PubSub+:

1. Authentication - Who can connect to a Solace PubSub+ instance
2. Authorization - What can a user do once connected to Solace

For authentication, Solace PubSub+ provides multiple authentication mechanisms.

1. BASIC - An internal database of Usernames/Passwords that a user provides on connection to Solace PubSub+. We will be using the BASIC Authentication mechanism for this course.
2. LDAP
3. Kerberos
4. OAuth (At the time of writing this codelab, this authentication mechanism is only partially supported)

Once a user is authenticated using any of the above mechanisms, the Authorization workflow takes place. There are three Authorization paradigms that are available to you via Solace PubSub+ via Access Control Lists (ACLs).

1. Connect ACL - Which IP Addresses are allowed to connect to the broker
2. Subscribe ACL - What topics are allowed to be subscribed to
3. Publish ACL - What topics are allowed to be published to

Note that each of the Access Control Lists can operate in a blacklist(the items specified are not allowed)/whitelist(the items specified are allowed)

Going back to Lesson 1, you used the default username password to connect to Solace either via Solace Cloud or your local docker instance.

To understand, what Access Control Lists map to this user you will navigate to the Administrative Interface for your broker.

If your broker is hosted in Solace Cloud, you have to navigate to the Manage tab and then :

(assets/lesson-05-solace-cloud-acl.png "Solace Cloud ACL")

If you are running a docker container locally, you can access the administrative interface by going to [http://localhost:8080] -> default -> Access Control

Once you are in the Access Control section, navigate to the 'Client Username' section:

(assets/lesson-05-solace-cloud-username.png "Solace Cloud Username")

You will see the `solace-cloud-client` you used to connect to the Solace PubSub+ Broker.

In addition, you can take a look at the `ACL Profiles`

(assets/lesson-05-solace-acl.png "Solace ACL")

Clicking into the `default` ACL profile will present you with the ACL settings.

In the `Client Connect` tab you will see that the `Client Connect Default Action` is set to `Allow` and the Exception List is impty which means that any ip-address who is authenticated can connect to the Broker.

(assets/lesson-05-solace-connect-acl.png "Solace ACL")

In the `Publish Topic` ACL, you will see that the `Publish Topic Default Action` is set to `Allow` which means that the `solace-cloud-client` user can `Publish` to any topic.

(assets/lesson-05-solace-publish-acl.png "Solace Publish ACL")

Likewise, in the `Subscribe Topic` ACL, you will see that the `Subscribe Topic Default Action` is set to `Allow` which means that the `solace-cloud-client` user can `Subscribe` to any topic.

(assets/lesson-05-solace-subscribe-acl.png "Solace Subscribe ACL")

As you may have come to realize, having Player1 and Player2 share ACL profiles is not a good idea since they can potentially both send false events to the game. To rectify this, we will be creating seperate users for the three application components : Player1, Player2 and the Controller all with restricted ACLs.

### Creating seperate ACL Profiles for the controller, Player1, and Player2

Use the [video](https://www.youtube.com/embed/QIUxCO78T3c) as a guide to create the following ACLs:

### 1. solace-battleship-controller-acl

1. Set the `Client Connect Default Action` to `Allow`
2. Set the following `Publish Topic Exceptions`:

```
SOLACE/BATTLESHIP/*/GAME/START
```

3. Set the following `Subscribe Topic Exceptions`:

```
SOLACE/BATTLESHIP/*/BOARD/SET/*
SOLACE/BATTLESHIP/*/JOIN/*
```

### 2. solace-battleship-player1-acl

1. Set the `Client Connect Default Action` to `Allow`
2. Set the following `Publish Topic Exceptions`:

```
SOLACE/BATTLESHIP/*/BOARD/SET/Player1
SOLACE/BATTLESHIP/*/JOIN/Player1
SOLACE/BATTLESHIP/*/Player1/MOVE
SOLACE/BATTLESHIP/*/Player2/MOVE-REPLY
```
...
```
SOLACE/BATTLESHIP/*/GAME/START
SOLACE/BATTLESHIP/*/Player2/MOVE
SOLACE/BATTLESHIP/*/Player1/MOVE-REPLY
SOLACE/BATTLESHIP/*/BOARD/SET/*
```

### 3. solace-battleship-player2-acl

1. Set the `Client Connect Default Action` to `Allow`
2. Set the following `Publish Topic Exceptions`:

```
SOLACE/BATTLESHIP/*/BOARD/SET/Player2
SOLACE/BATTLESHIP/*/JOIN/Player2
SOLACE/BATTLESHIP/*/Player2/MOVE
SOLACE/BATTLESHIP/*/Player1/MOVE-REPLY
```
...
You now have created the ACL Profiles for all of the application components.

### Creating seperate users for the contoller, Player1, and Player2

Use the [video](https://www.youtube.com/embed/QIUxCO78T3c) as a guide to create the following users:

#### solace-battleship-contoller

1. Enable the User
2. Enter a password `contoller` for the component
3. Select the ACL profile `solace-battleship-contoller` from the dropdown

#### solace-battleship-player1

1. Enable the User
2. Enter a password `player1` for the component
3. Select the ACL profile `solace-battleship-player1` from the dropdown

#### solace-battleship-player2

1. Enable the User
2. Enter a password `player2` for the component
3. Select the ACL profile `solace-battleship-player2` from the dropdown

You have now created all the users required for this app at this point and you can now move onto modifying your code.

### Modifiying your game-config.ts with the additional credentials

Modify your game-config.ts as follows (filling in the details for hte Secure Websocket Host and solace_vpn):

```ts
export const gameConfig = {
  solace_hostUrl: "", // Secure Websocket Host
  solace_vpn: "", // Solace VPN
  solace_battleship_users: {
    contoller: {
      solace_userName: "solace-battleship-contoller",
      solace_password: "contoller"
    },
    Player1: {
      solace_userName: "solace-battleship-player1",
      solace_password: "player1"
    },
    Player2: {
      solace_userName: "solace-battleship-player2",
      solace_password: "player2"
    }
  },
  allowed_ships: 5,
  gameboard_dimensions: 5
...
### Modifying your code to pass in the username upon connect

Navigate to `src\battleship_frontend\controller-app/landing-page.ts` and modify the lines from:

```
  // Connect to Solace with the Controller user
...
```
  // Connect to Solace with the Controller user
    this.solaceClient.connect("Controller").then(() =>
```

and then in `src/battleship_frontend/player-app/join.ts`

```
  //Connect to Solace with the playername
    this.solaceClient
      .connect()
```
...
```
  //Connect to Solace with the playername
    this.solaceClient
      .connect(this.player.name)
```

Navigate to [http://localhost:12345](http://localhost:12345) and your game should work as before but with improved security!

### Summary

In this lesson you learned about using different user's and Access Control Lists to determine what a user is allowed to publish and subscribe to in Solace PubSub+.

Be sure to commit the changes you made to this branch by running `git commit -m "optional1"`

To see the completed code for this section, sync the following branch [battleship-optional-1-creating-seperate-users-solution](https://github.com/solacese/battleship/blob/battleship-lesson-5-creating-seperate-users-solution) using the command:

`git checkout battleship-optional-1-creating-seperate-users-solution`
...
![Topic Dispatch](assets/solace-client-wrapper-topic-dispatch.gif)
